...
- Mandatory policies by route:
  ...
  | `POST /catalog/products/{id}/image` | 5 req | 1 min |
...
- All three pillars are mandatory:
  - **Logs** → Seq + Loki (local, both fed by Serilog; Loki is also a Grafana datasource) / Azure Monitor (production)
...
- Smoke tests against the live Docker stack go in `Ecommerce.SmokeTests` (no TestContainers — exercises the real running services).
